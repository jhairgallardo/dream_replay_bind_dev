...
from models.seek_maskingV2 import Seek_Network
...
from utils import seed_everything_with_fabric, draw_unmasked_badge
...
parser.add_argument('--act_enc_n_heads', type=int, default=1)
### Seek parameters
...
parser.add_argument('--seek_n_layers', type=int, default=2)
parser.add_argument('--seek_n_heads', type=int, default=3)
...
parser.add_argument('--bind_n_heads', type=int, default=2)
...
parser.add_argument('--coeff_std', type=float, default=0.1)
...
parser.add_argument('--shadow_view_encoder_after_generator', action='store_true', default=False)
### Other parameters
...
def std_loss(x):
    """
    Variance Regularization Loss (VICReg style).
    x: Tensor of shape (B, V, T, D) or (N, D)
    
    Forces the standard deviation of EACH feature dimension to be at least 1.0.
    This prevents "Dimensional Collapse" where the network uses only a few 
    spiking dimensions to represent everything.
    """
    # 1. Flatten to (Total_Samples, D)
    if x.ndim > 2:
        x = x.reshape(-1, x.shape[-1])
    
    # 2. Calculate Standard Deviation per Feature Dimension (Column-wise)
    # var(dim=0) calculates variance across the batch/time dimension
    std = torch.sqrt(x.var(dim=0) + 1e-4)
    
    # 3. Hinge Loss: penalize if std < 1.0
    # F.relu(1 - std) is 0 if std >= 1, and (1-std) if std < 1
...
def adjust_ema_momentum_linear(current_step, total_steps, initial_momentum, final_momentum=1.0):
    """
    Linear schedule for EMA momentum (IJEPA Style).
    Linearly increases from initial_momentum to final_momentum.
    """
    # Cap current step at total_steps to avoid overshooting
    # (though usually the loop ends exactly at total_steps)
    step = min(current_step, total_steps)
    # Linear interpolation
    momentum = initial_momentum + (final_momentum - initial_momentum) * (step / total_steps)
    return momentum

def normalized_mse_loss(pred, target):
    # pred, target: (B, N, D)
    
    # 1. Normalize both vectors to lie on the unit hypersphere
    # This kills the magnitude drift. The network can't "cheat" by blowing up norms.
    pred_norm = F.normalize(pred, dim=-1, p=2)
    target_norm = F.normalize(target, dim=-1, p=2)
    
    # 2. Calculate MSE on the sphere
    # This is mathematically proportional to Cosine Distance: ||x-y||^2 = 2 - 2<x,y>
...
def main():
    ...
    if args.shadow_view_encoder_after_generator and args.freeze_view_encoder:
        raise ValueError("Shadow view encoder after generator and freeze view encoder are True. This is not allowed. Pick one or the other.")
    ...
    if args.shadow_view_encoder_after_generator:
        view_encoder_shadow = copy.deepcopy(view_encoder)
        for p in view_encoder_shadow.parameters():
            p.requires_grad = False
        view_encoder_shadow.eval()
        # Shadow view encoder lives on device but is not wrapped with DDP or optimizer
        view_encoder_shadow.to(fabric.device)
    ...
    for epoch in range(args.epochs):
        ...
        train_loss_std = MetricLogger('Train Loss STD')
        ...
        for i, (batch_episodes, batch_labels) in enumerate(train_loader):
            ...
            noflat_PRED_imgfttoks, views_mask_indices = seek(noflat_acttok, noflat_imgfttoks, noflat_ret2D, crop_bv) # (B, V, Timg, Dimg), (B, V)
            ...
            if args.shadow_view_encoder_after_generator:
                # Sync Shadow Encoder to Current Student
                view_encoder_shadow.load_state_dict(view_encoder.state_dict())
                # Ensure Shadow matches Student's mode (train/eval) for Dropout consistency
                if view_encoder.training:view_encoder_shadow.train()
                else:view_encoder_shadow.eval()
                flat_PRED2_imgfttoks, _ = view_encoder_shadow(flat_PRED_imgs) # (B*V, Timg, D) # This one uses the shadow view encoder
            else:
            ...
            with fabric.autocast(): # Run losses calculations in mixed precision (models already run in mixed precision)
                if args.ema_momentum_view_encoder is not None:
                    ...
                    loss_mse_1 = normalized_mse_loss(noflat_PRED_imgfttoks[views_mask_indices], noflat_imgfttoks_ema_targets_detach[views_mask_indices])
                    loss_mse_2 = normalized_mse_loss(noflat_PRED2_imgfttoks[views_mask_indices], noflat_imgfttoks_ema_targets_detach[views_mask_indices])
                else:
                    # Reconstruction losses in latent space (no ema)
                    noflat_imgfttoks_detach = noflat_imgfttoks.detach()
                    loss_mse_1 = normalized_mse_loss(noflat_PRED_imgfttoks[views_mask_indices], noflat_imgfttoks_detach[views_mask_indices])
                    loss_mse_2 = normalized_mse_loss(noflat_PRED2_imgfttoks[views_mask_indices], noflat_imgfttoks_detach[views_mask_indices])
                
                # STD loss (VICReg style variance loss)
                loss_std = std_loss(noflat_imgfttoks)
                ...
                loss_total = args.coeff_mse * loss_mse_total + args.coeff_bce * loss_bce + args.coeff_std * loss_std
            ...
            if args.ema_momentum_view_encoder is not None:
                ## Adjust momentum value linearly
                current_momentum = adjust_ema_momentum_linear(current_step, total_steps, args.ema_momentum_view_encoder, final_momentum=1.0)
                ## Update EMA view encoder
                update_ema(view_encoder, view_encoder_ema, current_momentum)
            ...
            train_loss_std.update(fabric.all_reduce(loss_std.detach(), reduce_op="mean").item(), B_allranks)
            ...
            if fabric.is_global_zero and ((i % args.print_frequency) == 0):
                fabric.log(name=f'Loss Total', value=train_loss_total.val, step=current_step)
                fabric.log(name=f'Loss MSE Total', value=train_loss_MSE_total.val, step=current_step)
                fabric.log(name=f'Loss BCE', value=train_loss_BCE.val, step=current_step)
                fabric.log(name=f'Loss MSE 1', value=train_loss_MSE_1.val, step=current_step)
                fabric.log(name=f'Loss MSE 2', value=train_loss_MSE_2.val, step=current_step)
                fabric.log(name=f'Loss STD', value=train_loss_std.val, step=current_step)
                fabric.log(name=f'Top1 ACC', value=train_acc1.val, step=current_step)
                fabric.log(name=f'Top5 ACC', value=train_acc5.val, step=current_step)
                fabric.log(name=f'lr_venc', value=scheduler.get_last_lr()[0], step=current_step)
                if args.ema_momentum_view_encoder is not None:
                    fabric.log("EMA Momentum", current_momentum, step=current_step)
                fabric.print(
                    ...
                    f'Loss STD: {train_loss_std.val:.6f} -- '
        ...
        fabric.log(name=f'Loss STD (per epoch)', value=train_loss_std.avg, step=epoch)
        ...
        fabric.print(
            ...
            f'Loss STD: {train_loss_std.avg:.6f} -- '
        ...
        val_loss_std = MetricLogger('Val Loss STD')
        ...
        with torch.no_grad():
            for j, (batch_episodes, batch_labels) in enumerate(val_loader):
                ...
                if j==0 and fabric.is_global_zero:
                    ...
                    tok_max_abs = torch.amax(torch.abs(flat_imgfttoks), dim=(0, 1)).detach().float().cpu().numpy()  # (D,)
                    tok_min_abs = torch.amin(torch.abs(flat_imgfttoks), dim=(0, 1)).detach().float().cpu().numpy()  # (D,)
                    ...
                    axs[3].bar(dims, tok_max_abs)
                    axs[3].set_title('PatchToken (View encoder) max absolute value per dimension')
                    axs[3].set_xlabel('Dimension')
                    axs[3].set_ylabel('Max Absolute Value')
                    axs[4].bar(dims, tok_min_abs)
                    axs[4].set_title('PatchToken (View encoder) min absolute value per dimension')
                    axs[4].set_xlabel('Dimension')
                    axs[4].set_ylabel('Min Absolute Value')
                    ...
                    tok_max_abs = torch.amax(torch.abs(noflat_PRED_imgfttoks), dim=(0, 1, 2)).detach().float().cpu().numpy()  # (D,)
                    tok_min_abs = torch.amin(torch.abs(noflat_PRED_imgfttoks), dim=(0, 1, 2)).detach().float().cpu().numpy()  # (D,)
                    dims = np.arange(tok_means.shape[0])
                    fig, axs = plt.subplots(5, 1, figsize=(12, 18), constrained_layout=True)
                    axs[0].bar(dims, tok_means)
                    ...
                    axs[2].set_ylabel('Mean Absolute Value')
                    axs[3].bar(dims, tok_max_abs)
                    axs[3].set_title('PatchToken (Seek) max absolute value per dimension')
                    axs[3].set_xlabel('Dimension')
                    axs[3].set_ylabel('Max Absolute Value')
                    axs[4].bar(dims, tok_min_abs)
                    axs[4].set_title('PatchToken (Seek) min absolute value per dimension')
                    axs[4].set_xlabel('Dimension')
                    axs[4].set_ylabel('Min Absolute Value')
                    save_plot_dir = os.path.join(args.save_dir, 'Token_stats_val')
                    if not os.path.exists(save_plot_dir):
                    ...
                    tok_max_abs = torch.amax(torch.abs(noflat_PRED2_imgfttoks), dim=(0, 1, 2)).detach().float().cpu().numpy()  # (D,)
                    tok_min_abs = torch.amin(torch.abs(noflat_PRED2_imgfttoks), dim=(0, 1, 2)).detach().float().cpu().numpy()  # (D,)
                    dims = np.arange(tok_means.shape[0])
                    fig, axs = plt.subplots(5, 1, figsize=(12, 18), constrained_layout=True)
                    axs[0].bar(dims, tok_means)
                    ...
                    axs[2].set_ylabel('Mean Absolute Value')
                    axs[3].bar(dims, tok_max_abs)
                    axs[3].set_title('PatchToken (Generator+View encoder) max absolute value per dimension')
                    axs[3].set_xlabel('Dimension')
                    axs[3].set_ylabel('Max Absolute Value')
                    axs[4].bar(dims, tok_min_abs)
                    axs[4].set_title('PatchToken (Generator+View encoder) min absolute value per dimension')
                    axs[4].set_xlabel('Dimension')
                    axs[4].set_ylabel('Min Absolute Value')
                    save_plot_dir = os.path.join(args.save_dir, 'Token_stats_val')
                    if not os.path.exists(save_plot_dir):
                    ...
                    tok_max_abs = torch.amax(torch.abs(flat_acttok), dim=(0, 1)).detach().float().cpu().numpy()  # (D,)
                    tok_min_abs = torch.amin(torch.abs(flat_acttok), dim=(0, 1)).detach().float().cpu().numpy()  # (D,)
                    dims = np.arange(tok_means.shape[0])
                    fig, axs = plt.subplots(5, 1, figsize=(12, 18), constrained_layout=True)
                    axs[0].bar(dims, tok_means)
                    ...
                    axs[2].set_ylabel('Mean Absolute Value')
                    axs[3].bar(dims, tok_max_abs)
                    axs[3].set_title('ActionToken (Action encoder) max absolute value per dimension')
                    axs[3].set_xlabel('Dimension')
                    axs[3].set_ylabel('Max Absolute Value')
                    axs[4].bar(dims, tok_min_abs)
                    axs[4].set_title('ActionToken (Action encoder) min absolute value per dimension')
                    axs[4].set_xlabel('Dimension')
                    axs[4].set_ylabel('Min Absolute Value')
                    save_plot_dir = os.path.join(args.save_dir, 'Token_stats_val')
                    if not os.path.exists(save_plot_dir):
                ...
                val_loss_std.update(fabric.all_reduce(loss_std.detach(), reduce_op="mean").item(), B_allranks)
        ...
        fabric.log(name=f'Val Loss STD (per epoch)', value=val_loss_std.avg, step=epoch)
        ...
        fabric.print(
                ...
                f'Loss STD: {val_loss_std.avg:.6f} -- '
        ...
        if fabric.is_global_zero:
            if (epoch+1) % 1 == 0 or epoch==0:
                ...
                with torch.no_grad():
                    ...
                    noflat_PRED_imgfttoks, views_mask_indices = seek(noflat_acttok, noflat_imgfttoks, noflat_ret2D, crop_bv, selected_view=epoch%V) # (N, V, Timg, Dimg), (N, V)
                ...
                for i in range(N):
                    ...
                    # Write "unmasked" on the unmasked views
                    unmasked_flags = (~views_mask_indices[i]).to(episode_i_imgs.device)
                    for v in range(V):
                        if unmasked_flags[v]:
                            episode_i_imgs[v] = draw_unmasked_badge(episode_i_imgs[v])
                ...
                with torch.no_grad():
                    ...
                    noflat_PRED_imgfttoks, views_mask_indices = seek(noflat_acttok, noflat_imgfttoks, noflat_ret2D, crop_bv, selected_view=epoch%V) # (N, V, Timg, Dimg), (Timg)
                    # Generator forward pass
                    noflat_PRED_imgs = generator(noflat_PRED_imgfttoks) # (N, V, C, H, W)
                episodes_plot_gen_imgs = noflat_PRED_imgs.detach().cpu() # (N, V, C, H, W)
                episodes_plot_imgs = episodes_plot_imgs.detach().cpu() # (N, V, C, H, W)
